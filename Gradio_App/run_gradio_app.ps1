# ============================================================
#  AI Social Listening - Gradio Web UI launcher (PowerShell)
...
& $venvPython -c "import transformers, torch, gradio" 2>$null
if ($LASTEXITCODE -ne 0) {
    Write-Host "[ERROR] Missing packages in .venv (transformers/torch/gradio)." -ForegroundColor Red
    Write-Host ""
...
Write-Host "[INFO] Starting Gradio from the project virtual environment..."
Push-Location $projectRoot
try {
    & $venvPython gradio_app.py @args
}
finally {
    Pop-Location
}