...
class VietnameseSentimentAnalyzer:
    ...
    def _load_transformer_model(self):
        """
        Loads the PhoBERT Hugging Face classification pipeline ONCE per process.

        The pipeline is cached module-wide (keyed by MODEL_NAME) and reused by
        every VietnameseSentimentAnalyzer instance, so no module/session ever
        builds a duplicate in-memory model. A module-level lock prevents a
        double-load race when multiple threads/sessions start concurrently.
        HuggingFace's own disk cache ensures the first load (only) may download,
        and subsequent processes reuse the local copy without re-downloading.
        """
        with _MODEL_CACHE_LOCK:
            ...
            if cached is not None:
                ...
                logger.info(f"[VietnameseSentimentAnalyzer] Reusing cached PhoBERT pipeline for '{self.MODEL_NAME}'.")
                return

            try:
                logger.info(f"[VietnameseSentimentAnalyzer] Initializing PhoBERT model '{self.MODEL_NAME}'...")
                ...
                self.is_transformer_loaded = True
                self.load_error = None
                logger.info("[VietnameseSentimentAnalyzer] PhoBERT model successfully loaded into memory!")
            except Exception as e:
                self.load_error = str(e)
                logger.warning(f"[VietnameseSentimentAnalyzer] Could not load PhoBERT Transformer model: {e}")
